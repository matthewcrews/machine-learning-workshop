...
//
// The file "KNN.fsx" contains an implementation of distance,
// as well as a a Nearest Neighbor classifier.
...
// 'getFeatureVector' on the given 'text', and we can then 
// directly use the classify function from the KNN.fsx file,
// to find the language that has the most similar features.

let classifyLanguage text =
    let features = getFeatureVector text
    classify languageFeatures features
...
// ----------------------------------------------------------------------------
// Bonus: visualize the language features using Principal Component Analysis (PCA)
// Read up on PCA here: 
