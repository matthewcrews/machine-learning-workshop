...
let fakeWeights = Array.init (features.Length + 1) (fun _ -> 0.)
...
let gradient_example = steps 10 0.5 6.0
...
let error trainingData weights = 
    ...
    |> Array.map (fun (label,features) -> 
        label, predict weights features)
    |> Array.sumBy (fun (label,predicted) -> 
        pown (label - predicted) 2)
...
[ Seq.zip features trained ]
|> Chart.Column
|> Chart.WithLabels ["Weights"]
